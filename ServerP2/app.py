...
def classifyYelp():
	...
	with open('ServerP2/models/Predict/tokenizer.pickle', 'rb') as handle:
		tokenizer = pickle.load(handle)

	sequences = tokenizer.texts_to_sequences([text])
	data = pad_sequences(sequences, maxlen=500)

	new_model = keras.models.load_model('ServerP2/models/Predict/fakeYelp.h5')
...
def file_search(section, productID):
	...
	if category == "steam":
		weights = "ServerP2/models/steam/" + productID + ".hdf5"
	elif category == "yelp":
		weights = "ServerP2/models/yelp/" + productID + ".hdf5"
